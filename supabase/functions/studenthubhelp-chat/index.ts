...
function isPropertyQuery(msg:string,history:string){const x=n(msg),h=n(history),currentExplicit=!!cat(msg)||!!secondaryCats(msg).length||!!findLandmark(msg)||/(property|listing|rent|room|hostel|pg|tiffin|mess|library|cafe|bookstore|ke paas|k pass|paas me|area|locality|landmark|owner|availability|verified)/i.test(x);if(currentExplicit)return true;const hasPropertyContext=!!cat(history)||/(hostel|pg|tiffin|mess|library|cafe|bookstore|property|listing)/i.test(h);return hasPropertyContext&&/^(show|more|another|same|this|that|yes|haan|ha|ok|okay|ac|wifi|non ac|veg|girls|boys|near|nearest|dikhao|dikhado|aur|options|option|wala|wali|chahiye|budget|under|below|within)\\b/i.test(x)}
...
Deno.serve(async(req:Request)=>{
 ...
 try{
  ...
  if(!propertyMode){const ar=await aiGeneral(msg,history);return new Response(JSON.stringify({reply:ar.reply||"Bilkul 😊 Bataiye, main aapki help karta hoon.",intent:"General / Study / Conversation",primaryTopic:"Student AI Assistant",recommendedProperties:[],recommendations:[],suggestedFollowUps:["Ask a study question","Tell me something interesting","Find a property","Contact support"],grounded:true,webGrounded:ar.webGrounded||false,webSources:ar.webSources||[],sessionId}),{headers:H})}
