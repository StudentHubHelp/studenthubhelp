...
function isPropertyQuery(msg:string,history:string){const x=n(msg+" "+history);return !!cat(msg)||!!secondaryCats(msg).length||/(property|listing|rent|room|hostel|pg|tiffin|mess|library|cafe|bookstore|nearby|near me|ke paas|k pass|paas me|area|locality|landmark|owner|availability|verified)/i.test(x)}
