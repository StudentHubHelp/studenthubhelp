...
export const PropertiesView: React.FC<PropertiesViewProps> = ({
    ...
    return sourceProperties
      .filter((p) => {
        ...
        const matchesOwner =
          ...
          (p.phone || '').includes(ownerFilter) ||
          (p.name || '').toLowerCase().includes(ownerFilter.toLowerCase());
  ...
  const toggleSelectAll = () => {
    if (selectedIds.length === filteredProperties.length) {
      ...
      setSelectedIds(filteredProperties.map((p) => String(p.id)));
    }
  };
  ...
  const pageTitle =
    categoryFilter === 'all'
      ...
      : propertyConfig[categoryFilter as PropertyType]?.title || 'Properties';

  const defaultAddType: PropertyType =
    categoryFilter !== 'all' ? (categoryFilter as PropertyType) : 'hostels';
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <button
            onClick={() => exportToCSV(filteredProperties, `properties-${selectedCategory}`)}
          ...
          <button
            onClick={() => onOpenEditor(null, defaultAddType)}
            className="px-4 py-2 rounded-xl bg-gradient-to-r from-amber-400 to-amber-500 hover:from-amber-300 hover:to-amber-400 text-slate-950 text-xs font-extrabold flex items-center gap-1.5 shadow-lg transition-all cursor-pointer"
          >
      ...
      <div className="rounded-3xl bg-[#081026] border border-slate-800 p-4 space-y-3 shadow-xl">
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-7 gap-2.5 text-xs">
          {categoryFilter === 'all' && (
            <select
              ...
              className="bg-[#0d1838] border border-slate-700 rounded-xl p-2.5 text-white"
            >
          ...
          <input
            ...
            className="bg-[#0d1838] border border-slate-700 rounded-xl p-2.5 text-white placeholder-slate-400"
          />
        ...
        {selectedIds.length > 0 && (
          <div className="flex flex-wrap items-center justify-between gap-2 p-2.5 rounded-xl bg-[#0d1838] border border-amber-500/30 text-xs animate-in fade-in duration-150">
            ...
            <div className="flex items-center gap-1.5">
              <button
                onClick={() => onBulkAction('verify', selectedIds)}
                className="px-3 py-1.5 rounded-lg bg-emerald-500/20 text-emerald-300 hover:bg-emerald-500/30 font-bold"
              >
                Bulk Verify
              </button>
              <button
                onClick={() => onBulkAction('activate', selectedIds)}
                className="px-3 py-1.5 rounded-lg bg-blue-500/20 text-blue-300 hover:bg-blue-500/30 font-bold"
              >
                Bulk Activate
              </button>
              <button
                onClick={() => onBulkAction('suspend', selectedIds)}
                className="px-3 py-1.5 rounded-lg bg-rose-500/20 text-rose-300 hover:bg-rose-500/30 font-bold"
              >
                Bulk Suspend
              </button>
              <button
                onClick={() => onBulkAction('feature', selectedIds)}
                className="px-3 py-1.5 rounded-lg bg-amber-500/20 text-amber-300 hover:bg-amber-500/30 font-bold"
              >
                Bulk Feature
              </button>
            </div>
                ...
                filteredProperties.map((p) => {
                  ...
                  return (
                    ...
                    >
                      {/* Checkbox */}
                      <td className="p-4">
                        <button onClick={() => toggleSelectOne(String(p.id))} className="text-slate-400 hover:text-white">
                          {isSelected ? (
                            <CheckSquare className="w-4 h-4 text-amber-400" />
                          ) : (
                            <Square className="w-4 h-4" />
                      ...
                      <td className="p-4 font-mono">
                        <div className="font-sans font-medium text-slate-200">{p.owner_name || '—'}</div>
                      ...
                      <td className="p-4 text-amber-300 font-bold">
                        {propRating(p) > 0 ? (
                          <span className="flex items-center gap-1">
                            <Star className="w-3 h-3 fill-amber-300" /> {propRating(p)}
                          </span>
                        ) : (
