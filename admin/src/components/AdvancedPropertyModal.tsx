...
export const AdvancedPropertyModal: React.FC<AdvancedPropertyModalProps> = ({
  ...
  useEffect(() => {
    if (property) {
      ...
      for (const field of columns) next[field] = (property as any)[field] ?? null;
      setFormData(next);
    } else {
      const next: Record<string, any> = {};
      for (const field of columns) next[field] = null;
      next.category = config.category;
      setFormData(next);
    }
    setError(null);
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      // The save layer uses this internal field only to select the exact Supabase table.
  ...
  return (
    <div
      className="fixed inset-0 z-[500] flex items-center justify-center p-3 sm:p-5 bg-black/80 backdrop-blur-md"
      onClick={(e) => {
    ...
    >
      <div className="relative w-full max-w-6xl max-h-[94vh] flex flex-col rounded-3xl overflow-hidden shadow-2xl border border-amber-500/30 bg-[#081026] text-white">
        <div className="p-5 border-b border-slate-800 flex items-center justify-between gap-4 bg-[#0a1430] shrink-0">
          <div>
            <div className="text-[10px] uppercase tracking-widest text-amber-400 font-extrabold flex items-center gap-1.5">
              ...
              {isNew ? 'Add Property' : 'Master Property Editor'}
            </div>
            <h2 className="text-xl font-bold mt-1">{isNew ? `Add New ${config.singular}` : formData.name || 'Edit Property'}</h2>
            <p className="text-xs text-slate-400 mt-1">
              Live Supabase table: <span className="text-amber-300 font-mono">{selectedType}</span> · {columns.length} exact DB columns
            </p>
          </div>
          <button type="button" onClick={onClose} className="w-10 h-10 rounded-full bg-slate-800 text-slate-400 hover:text-white flex items-center justify-center">
            <X className="w-5 h-5" />
          </button>
        </div>
              ...
              return (
                <div key={field} className={isTextarea ? 'sm:col-span-2 lg:col-span-3' : ''}>
                  <label className="text-xs font-bold text-slate-300">
                    {labelFor(field)}{field === 'id' && isNew ? ' (DB generated)' : required ? ' *' : ''}
                  </label>
                  ...
                  {isBoolean ? (
                    <select
                      ...
                      disabled={isReadonly}
                      onChange={(e) => handleChange(field, normalizeInputValue(field, e.target.value))}
                      className="w-full mt-1 bg-[#0d1838] border border-slate-700 rounded-xl px-3.5 py-2.5 text-sm text-white"
                    >
                    ...
                    <input
                      ...
                      value={valueForInput}
                      readOnly={isReadonly || (field === 'id' && isNew)}
                      required={required}
                      onChange={(e) => handleChange(field, normalizeInputValue(field, e.target.value))}
        ...
        <div className="p-4 sm:p-5 border-t border-slate-800 bg-[#0a1430] flex items-center justify-between gap-3 shrink-0">
          <button type="button" onClick={onClose} className="px-5 py-2.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-bold">
            Cancel
          </button>
          <button type="submit" form="property-master-editor-form" disabled={saving} className="px-6 py-2.5 rounded-xl bg-gradient-to-r from-amber-400 to-amber-500 text-slate-950 text-xs font-extrabold flex items-center gap-2 disabled:opacity-50">
            {saving ? 'Saving to Supabase...' : <><Save className="w-4 h-4" /> Save Property Live</>}
          </button>
        </div>
      </div>
    </div>
