...
const statusClass = (status?: string) => {
  ...
  if (s === 'approved' || s === 'published' || s === 'active' || s === 'resolved') {
...
const Badge = ({ status }: { status?: string }) => (
  <span className={`text-[10px] font-bold px-2 py-1 rounded-full border capitalize ${statusClass(status)}`}>
    {status || 'pending'}
  </span>
);
...
export const PlatformControlViews: React.FC<PlatformControlViewsProps> = (props) => {
  ...
  const ownerById = useMemo(() => new Map(owners.map((o) => [String(o.id), o])), [owners]);
  ...
  const pendingListings = listingRequests.filter((r) => {
    ...
    return [r.name, r.owner_name, r.phone, r.area, r.category]
  ...
  if (currentSubTab === 'listing-requests') {
    ...
    const reject = async (id: string | number) => {
      ...
      try {
        ...
        alert(e?.message || 'Unable to reject listing request.');
        setBusyId(null);
      }
    };
      ...
      <div className="space-y-6 animate-in fade-in duration-200">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">Only pending requests appear here. Approved listings move to their real property table; rejected requests leave this queue.</p>
          </div>
          <div className="flex gap-2 items-center">
            <div className="relative">
              <Search className="absolute left-3 top-2.5 w-4 h-4 text-slate-500" />
              <input value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Search request..." className="pl-9 pr-3 py-2 rounded-xl bg-[#0d1838] border border-slate-700 text-xs text-white outline-none" />
            </div>
            <button onClick={() => exportToCSV(pendingListings, 'listing-requests-pending')} className="px-3 py-2 rounded-xl bg-[#0d1838] border border-amber-500/30 text-amber-300 text-xs font-bold flex items-center gap-1.5"><Download className="w-3.5 h-3.5" />CSV</button>
          </div>
        </div>
        <div className="rounded-3xl bg-[#081026] border border-slate-800 overflow-hidden shadow-xl">
          <div className="overflow-x-auto"><table className="w-full text-left text-xs">
            <thead className="bg-[#0d1838] text-amber-300 font-bold"><tr>{['Request','Property','Category','Owner','Phone','Area','Submitted','Status','Actions'].map((h) => <th key={h} className="p-4">{h}</th>)}</tr></thead>
            <tbody className="divide-y divide-slate-800/60">
              {pendingListings.length ? pendingListings.map((r) => <tr key={r.id} className="hover:bg-slate-800/40">
                <td className="p-4 font-mono text-slate-400">#{r.id}</td>
                <td className="p-4 font-bold text-white">{r.name || r.property_name || '—'}</td>
                <td className="p-4 text-slate-300">{r.category || r.property_type || 'Hostel'}</td>
                <td className="p-4 text-slate-300">{r.owner_name || '—'}</td>
                <td className="p-4 text-slate-300">{r.phone || r.owner_phone || '—'}</td>
                <td className="p-4 text-slate-300">{r.area || '—'}</td>
                <td className="p-4 text-slate-400">{shortDate(r.created_at || r.submitted_at)}</td>
                <td className="p-4"><Badge status="pending" /></td>
                <td className="p-4 whitespace-nowrap"><div className="flex gap-1.5">
                  <button disabled={busyId === r.id} onClick={() => approve(r.id)} className="px-3 py-1.5 rounded-lg bg-emerald-500/15 text-emerald-300 border border-emerald-500/30 font-bold flex items-center gap-1"><Check className="w-3.5 h-3.5" />Approve & Publish</button>
                  <button disabled={busyId === r.id} onClick={() => reject(r.id)} className="px-3 py-1.5 rounded-lg bg-rose-500/15 text-rose-300 border border-rose-500/30 font-bold flex items-center gap-1"><X className="w-3.5 h-3.5" />Reject</button>
                </div></td>
              </tr>) : <tr><td colSpan={9} className="p-10 text-center text-slate-400">No pending listing requests.</td></tr>}
            </tbody>
          </table></div>
        </div>
      </div>
  ...
  if (currentSubTab === 'claim-requests') {
    ...
    const approve = async (id: string | number) => {
      setBusyId(id);
      try {
        const { error } = await supabase.rpc('admin_approve_claim_request', { p_claim_id: Number(id) });
        if (error) throw error;
        onApproveClaim(id);
        setBusyId(null);
      } catch (e: any) {
        alert(e?.message || 'Claim approval failed. Ownership was not changed.');
        setBusyId(null);
      }
    };
    const reject = async (id: string | number) => {
      setBusyId(id);
      try {
        const { error } = await supabase.from('claim_requests').update({ status: 'rejected', updated_at: new Date().toISOString() }).eq('id', id).eq('status', 'pending');
        if (error) throw error;
        onRejectClaim(id);
        setBusyId(null);
      } catch (e: any) {
        alert(e?.message || 'Claim rejection failed.');
        setBusyId(null);
      }
    };
    return (
      <div className="space-y-6 animate-in fade-in duration-200">
        <div><h2 className="text-2xl font-serif font-extrabold text-white">Owner Claim Requests</h2><p className="text-xs text-slate-400 mt-1">A claim never grants ownership directly. It becomes an owner request first and only an admin approval changes the property owner.</p></div>
        <div className="rounded-3xl bg-[#081026] border border-slate-800 overflow-hidden shadow-xl"><div className="overflow-x-auto"><table className="w-full text-left text-xs">
          <thead className="bg-[#0d1838] text-amber-300 font-bold"><tr>{['Request','Property','Type','Claimant','Phone','Submitted','Status','Actions'].map((h) => <th key={h} className="p-4">{h}</th>)}</tr></thead>
          <tbody className="divide-y divide-slate-800/60">{pendingClaims.length ? pendingClaims.map((r) => <tr key={r.id} className="hover:bg-slate-800/40">
            <td className="p-4 font-mono text-slate-400">#{r.id}</td><td className="p-4 font-bold text-white">{r.property_name || propertyByKey.get(`${tableFor(r.property_type)}:${r.property_id}`)?.name || `Property #${r.property_id}`}</td><td className="p-4 text-slate-300">{r.property_type || '—'}</td><td className="p-4 text-slate-300">{r.claimant_name || r.owner_name || ownerById.get(String(r.claimant_id || ''))?.full_name || '—'}</td><td className="p-4 text-slate-300">{r.phone || (r as any).owner_phone || '—'}</td><td className="p-4 text-slate-400">{fmtDate(r.created_at)}</td><td className="p-4"><Badge status={r.status} /></td>
            <td className="p-4"><div className="flex gap-1.5"><button disabled={busyId === r.id} onClick={() => approve(r.id)} className="px-3 py-1.5 rounded-lg bg-emerald-500/15 text-emerald-300 border border-emerald-500/30 font-bold">Approve & Verify</button><button disabled={busyId === r.id} onClick={() => reject(r.id)} className="px-3 py-1.5 rounded-lg bg-rose-500/15 text-rose-300 border border-rose-500/30 font-bold">Reject</button></div></td>
          </tr>) : <tr><td colSpan={8} className="p-10 text-center text-slate-400">No pending ownership claims.</td></tr>}</tbody>
        </table></div></div>
      </div>
    );
  }
  ...
  if (currentSubTab === 'verification') {
    ...
    const process = async (id: string | number, status: 'approved' | 'rejected') => {
      setBusyId(id);
      try {
        const { error } = await supabase.rpc('admin_process_verification_request', { p_request_id: Number(id), p_status: status });
        if (error) throw error;
        if (status === 'approved') onApproveVerification(id); else onRejectVerification(id);
        setBusyId(null);
      } catch (e: any) {
        alert(e?.message || 'Verification request could not be processed.');
        setBusyId(null);
      }
    };
