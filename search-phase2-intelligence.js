(function(){
  ...
  // Adds strong multi-condition matching without replacing the existing search UI.
  const SYN={
    hostel:['hostel','pg','paying guest','payingguest','room','rooms','stay','accommodation'],
    tiffin:['tiffin','mess','dabba','home food','meal service','food delivery','lunch','dinner'],
    library:['library','study library','reading room','study room','self study','self-study'],
    cafe:['cafe','cafes','coffee shop','coffeehouse','restro','restaurant','eatery'],
    bookstore:['bookstore','book store','book shop','bookseller','book depot','stationery','stationary shop'],
    girls:['girls','girl','female','ladies','women','mahila','ladki','girls hostel','girls pg'],
    boys:['boys','boy','male','gents','men','purush','ladka','boys hostel','boys pg'],
    veg:['veg','vegetarian','pure veg','shakahari'],
    cheap:['cheap','affordable','budget','low price'],
  ...
  function priceOf(r){for(const k of ['price','monthly_price','monthly_rent','rent','monthly_fee','fee','amount']){const v=num(r?.[k]);if(v!=null)return v;}const p=fieldText(r).match(/(?:₹|rs\.?)[^0-9]{0,5}(\d[\d,]*)/i);return p?Number(p[1].replace(/,/g,'')):null;}
  function hasAny(v,arr){const x=norm(v);return !!x&&arr.some(a=>x.includes(norm(a)));}
  function conditionScore(r,q){
    const n=norm(q),g=groups(q),h=fieldText(r),cat=category(q);let s=0,matched=0,required=0;
