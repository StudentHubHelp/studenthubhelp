(function(){
  ...
  const FIELDS={
    exact:['name','property_name','service_name','title'],
    location:['area','city','address','nearby','landmark','location','locality','service_area'],
    attributes:['category','type','gender','food_type','meal_type','room_type','sharing','book_type','specialization','facilities','timing'],
    descriptive:['description','features','menu','details']
  };
  ...
  function scoreFinal(r,q,index){
    ...
    if(nq&&name.includes(nq)){bonus+=120;exact++;}
    ...
    if(verified)bonus+=8;
    const hasContact=!!(r?.phone||r?.contact_phone||r?.whatsapp||r?.email);if(hasContact)bonus+=2;
    const base=Number(r?.__score)||0;
    const phase2=Number(r?.__phase2Score)||0;
    const phase3=Number(r?.__phase3LocationScore)||0;
    const final=base+phase2+phase3+bonus;
    return {final,bonus,base,phase2,phase3,coverage,covered,parts:parts.length,exact,typo,verified};
  }
  function dedupe(rows){
    const seen=new Set(),out=[];
    for(const r of rows){
      const key=norm(r?.property_id||r?.id||nameOf(r));
      if(!key||seen.has(key))continue;seen.add(key);out.push(r);
    }
    return out;
  }
  function install(){
    ...
    window.ranked=function(data,q){
      ...
      const rows=dedupe(base).map((r,i)=>{const c=scoreFinal(r,q,i);return {...r,__phase4FinalScore:c.final,__phase4Confidence:c.coverage,__phase4Meta:c};});
      return rows.sort((a,b)=>b.__phase4FinalScore-a.__phase4FinalScore||b.__phase4Confidence-a.__phase4Confidence||String(nameOf(a)).localeCompare(String(nameOf(b))));
    };
    window.__studentHubPhase4={scoreFinal,queryParts,dedupe,version:'1.0.0'};
  }
