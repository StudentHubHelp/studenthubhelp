...
export const ChatWidget: React.FC<ChatWidgetProps> = ({
  ...
  const detailUrl = property?.detailsUrl;
  ...
  const facilities = [
    ...
    property?.open24Hours,
    property?.separateCabin
  ]
    .filter(Boolean)
    .map(String)
    .filter(v => v.toLowerCase() !== 'false');
  ...
  return (
    <div
      key={`${property?.id || property?.name || 'property'}-${index}`}
      className="rounded-2xl border border-slate-200 bg-white p-4 shadow-md space-y-3"
    >
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        {property?.verified && (
          <span className="shrink-0 text-[10px] font-bold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-1 rounded-full">
            ✓ Verified
          </span>
        )}
          ...
          <div className="flex flex-wrap gap-1.5">
            ...
            {highlights.map(
              (item: string, i: number) => (
                <span
                  key={i}
                  className="text-[10px] px-2 py-1 rounded-lg bg-amber-50 border border-amber-100 text-slate-700"
                >
                  {item}
                </span>
              )
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        {phone && (
          <a
            href={`tel:${phone}`}
            className="text-center px-3 py-2.5 rounded-xl bg-amber-400 text-slate-950 text-xs font-extrabold"
          >
        ...
        {property?.mapsUrl && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="text-center px-3 py-2.5 rounded-xl border border-slate-200 text-slate-700 text-xs font-extrabold"
          >
      ...
      {detailUrl && (
        <a
          href={detailUrl}
          className="block w-full text-center px-3 py-3 rounded-xl bg-slate-900 text-white text-xs font-extrabold"
        >
          View Full Details →
        </a>
      )}
