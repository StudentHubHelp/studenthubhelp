(function(){
  ...
  function normalizeQuery(q){
    const n=norm(q);const ts=tokens(n);const corrected=ts.map(typoToken);return {raw:String(q??''),normalized:n,tokens:ts,correctedTokens:corrected,corrected:corrected.join(' ')};
  }
  ...
  function parseIntent(q){
    ...
    const intent={category,groups:g,corrected:x.corrected,original:x.raw,hasTypoCorrection:x.corrected!==x.tokens.join(' '),near:/\b(near|nearby|paas|pas|pass|beside|next to|samne|saamne|opposite|across|piche|behind)\b/i.test(n),availability:g.includes('availability'),timing:g.includes('timing'),price:n.match(/(?:under|below|upto|up to|less than|below|within|kam|se kam|₹|rs\.?)[^0-9]{0,8}(\d[\d,]*(?:\.\d+)?)/i)?.[1]||null};
    return intent;
  }
  ...
  function semanticBonus(r,intent){
    ...
    for(const t of intent.correctedTokens){if(t.length<3)continue;if(h.includes(t))s+=9;else{const d=lev(t,h.slice(0,Math.min(h.length,220)));if(d<=1)s+=2;}}
  ...
  function install(){
    ...
    window.ranked=function(data,q){
      ...
      return base.map((r,i)=>({...r,__phase1Intent:intent,__phase1Bonus:semanticBonus(r,intent),__score:(r.__score||0)+semanticBonus(r,intent)})).sort((a,b)=>b.__score-a.__score||iName(a).localeCompare(iName(b)));
    };
    function iName(r){return String(r?.name||r?.title||r?.property_name||'');}
    window.__studentHubPhase1={normalizeQuery,parseIntent,lev,semanticBonus,version:'1.0.0'};
  }
