(function(){
  ...
  function normalizeQuery(q){const n=norm(q),ts=tokens(n),corrected=ts.map(typoToken);return {raw:String(q??''),normalized:n,tokens:ts,correctedTokens:Array.isArray(corrected)?corrected:[],corrected:corrected.join(' ')};}
  ...
  function semanticBonus(r,intent){
    ...
    for(const g of (Array.isArray(intent?.groups)?intent.groups:[])){const fs=fieldMap[g]||[];let hit=false;for(const f of fs){const v=norm(r?.[f]);if(v&&SYN[g].some(a=>v.includes(norm(a)))){hit=true;break;}}if(hit)s+=70;else if(SYN[g].some(a=>h.includes(norm(a))))s+=18;}
    for(const t of (Array.isArray(intent?.correctedTokens)?intent.correctedTokens:[])){if(t.length<3)continue;if(h.includes(t))s+=9;}
  ...
  function install(){
    ...
    window.__studentHubPhase1={normalizeQuery,parseIntent,lev,semanticBonus,version:'1.0.4'};
  }
