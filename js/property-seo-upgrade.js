(function(){
  ...
  function canonicalUrl(){
    ...
    u.searchParams.set('type',typeMap[type]?'':type);
    // Remove the temporary empty type and set the normalized application type.
  ...
  function setMeta(name,content,attr){
    if(!content) return;
    let el=document.querySelector(attr+'[name="'+name+'"]');
    if(!el){el=document.createElement('meta');el.setAttribute(attr,'name');el.setAttribute('name',name);document.head.appendChild(el)}
  ...
  function normalizeSchema(){
    ...
    if(!el) return false;
    let data;
    try{data=JSON.parse(el.textContent||'{}')}catch(_){return false}
    if(!data || !Array.isArray(data['@graph'])) return false;

    const business=data['@graph'].find(x=>x && (x['@type']==='LocalBusiness' || typeMap[type]===x['@type']));
    if(business){
      ...
      if(business.address){
        // Existing application data stores area/city in the two locality fields.
        // Preserve the area as extendedAddress and use the city as addressLocality.
        if(business.address.addressLocality && business.address.addressRegion){
      ...
      if(/Verified by StudentHubHelp/i.test(visible)) business.additionalProperty=[
        {'@type':'PropertyValue','name':'Verification status','value':'Verified by StudentHubHelp'}
      ];
      if(/Not currently verified/i.test(visible)) business.additionalProperty=[
        {'@type':'PropertyValue','name':'Verification status','value':'Not currently verified'}
      ];
    }
    ...
    if(page){
      ...
      page.mainEntity=business?{'@id':canonicalUrl()+'#business'}:undefined;
      if(!page.mainEntity) delete page.mainEntity;
    }
  ...
  function apply(){
    const hasIdentifier=Boolean(id||slug);
    if(!hasIdentifier){
      // The generic shell is not a real listing and should not become an indexable duplicate.
    ...
    setCanonical(url);
    setMeta('property-type',type,'property');
    normalizeSchema();
  }

  // Run before and after the existing property renderer updates JSON-LD.
  apply();
  const observer=new MutationObserver(()=>{if(normalizeSchema()) apply()});
